...
def getInput():
    if len(sys.argv) == 2:
        ...
        print("Please input the format as <lat1,lon1,lat2,lon2>.")
        sys.exit(1)
...
def Levelcal(lat1, lon1, lat2, lon2):
    LevelDic = {
    13: 2097152,
    14: 4194304,
    15: 8388608,
    ...
    p3 = [lat2, lon1]
    p4 = [lat2, lon2]
    SamplePoint_l = [p1, p2, p3, p4]
    listLevelOfCorner = list(map(getMaxLevelOfPoint, SamplePoint_l))
    ...
    for tmp in range(0, sampleNum):
        ...
        SamplePoint_l.append( [lat / exp, lon / exp] )

    levelOfDetail = max(list(map(getMaxLevelOfPoint, SamplePoint_l)))
...
def CreateImage(tileXY1, tileXY2, levelOfDetail):
    x1 = tileXY1[0]
    y1 = tileXY1[1]
    x2 = tileXY2[0]
    y2 = tileXY2[1]
...
def main():
    ...
    levelOfDetail = Levelcal(lat1, lon1, lat2, lon2)
    print("The bounded box level is %s." % (levelOfDetail))
    pixelXY1, tileXY1 = latLongToTileXY (lat1, lon1, 20)
    pixelXY2, tileXY2 = latLongToTileXY (lat2, lon2, 20)
    image = CreateImage(tileXY1, tileXY2, levelOfDetail)
    print("Image created.")
